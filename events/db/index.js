...
class DB {
  ...
  query(keyConditionExpression, expressionAttributeValues) {
    let self = this
    return new Promise((resolve, reject) => {
      let params = {
        TableName: applicationContext.db.model.TableName,
      ...
      self.docClient.scan(params, function(err, data) {
        if (err) {
          ...
          if (data && data.Items && data.Items.length > 0) {
            ...
            reject(new Error(`No records for ${JSON.stringify(keyConditionExpression)} with ${JSON.stringify(expressionAttributeValues)}`))
          }
        }
      });
    })
